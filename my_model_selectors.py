...
class SelectorBIC(ModelSelector):
    ...
    def select(self):
        ...
        for n in range(self.min_n_components, self.max_n_components+1):
            ...
            initial_state_occupation_probabilities = numStates
            transition_probabilities = numStates*(numStates - 1)
            emission_probabilities = numStates*numFeatures*2
            p = initial_state_occupation_probabilities + transition_probabilities + emission_probabilities
            bic = -2*logl + p * math.log(len(self.X))
            bics.append(bic)
        best_num_components = min(bics)
        return self.base_model(best_num_components)
        #except:
            #return None
        ...
        try: 
            ...
            for n in range(self.min_n_components, self.max_n_components+1):
                ...
                for cv_train_idx, cv_test_idx in split_method.split(self.sequences):
                    ...
                    try:
                        logs.append(model.score(X_test, lengths_test))
                    except:
                        return None
                avg_log.append(sum(logs)/len(logs))
            best_num_components = max(avg_log)
            return self.base_model(best_num_components)
        except:
            return None
